...
public class Kiosk {
            ...
            try {
                switch (page) {
                    case CATEGORY: // Category 출력
                        ...
                        throwIfOutOfBound(categoryIndex); // 범위를 벗어난 입력값에 대한 예외 체크
                    ...
                    case MAIN: // Main Menu 선택창
                        ...
                        Orders.printToAddOrNot(); //장바구니 추가 메세지 출력
                    ...
                    case ORDER: // Category 선택창에서 Order 선택시 나오는 Order 선택창
                        Orders.checkOrderOrNot(); //주문확인 선택창 출력

                        int payIndex = Integer.parseInt(scanner.nextLine());

                        if (payIndex == 1) {
                            Orders.terminateOrder();
    ...
    private void throwIfOutOfBound(int categoryIndex) throws Exception {
